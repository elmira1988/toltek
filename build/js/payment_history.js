...
function get_data_by_filter(page=1)  
{
	event.preventDefault();
	console.log("disabled");
	$("#paystatus").find("button").attr("disabled","disabled");
...
/*
		...
		function(info){	
			...
			var table=$('#datatable-fixed-header').DataTable( {
		        ...
		        "initComplete": function(settings, json) {
					...
					$("#paystatus").find("button").removeAttr("disabled");
				  }
		    } );
   ...
   arr.page=[page]; 
   $.post("build/php/get_table_payments_quickly.php",{data:JSON.stringify(arr)}, 
		function(info){	
			//console.log(info);  
			$("#datatable-fixed-header").find('tbody').html(info);
			//выводим количество записей
			$("#count_pay").text($("input[name='count_raw']").val());
        	
			//общая сумма
        	$("span#summa").html(""); 
        	if (!$("#paystatus").find("[name='notPay']").hasClass("active"))
			{
				$("span#summa").html(' &nbsp;&nbsp;&nbsp;&nbsp;<small>сумма </small>'+$("input[name='summ']").val().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ')); 	
			}
			//разблокировали кнопку
			$("#paystatus").find("button").removeAttr("disabled");			

			//инициализировали таблицу
			init_table($("#datatable-fixed-header")); 
		});		
...
{	
	...
	if ($("input[name='role']").val()==3)
		...
		{
			$("#receiving_button").css('display','block');	
		...
		{
			$("#receiving_button").css('display','none');	
		}
...
function change_blocks_of_filter()
{
	...
	{
		$("#paymasters").css("display","none");
	}
	else
	{
		$("#paymasters").css("display","block");
	}
