...
if ($id_students=save_data($query)) 
{
	...
	{
		$id_students_groups=save_data("INSERT INTO `students_groups` (`id_students`,`id_groups`,`year`,`months_of_study`) VALUE ($id_students,$id_groups[$i],".$GLOBALS['year'].",'".implode(",",get_months_between($month_begin,$month_end))."')"); 
	}
	...
	for ($i=0;$i<count($parents);$i++)
	{
		...
		array_push($arr_parents,save_data("INSERT INTO `parents` (".implode(",",$arr[0]).") VALUES (".implode(",",$arr[1]).")")); 
	}


	echo json_encode(array( "result" => "ok", "id_students" => $id_students, "id_parents" => $arr_parents));
}
else
{
	echo json_encode(array( "result" => "erroor"));
} 
 ?>